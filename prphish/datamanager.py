...
def selectcampaign_post():
    ...
    campaign = db.session.query(Campaign, EmailTemplate).filter_by(id=campaignId).join(EmailTemplate).join(EmailResponse)
    ...
    for x in responseTypes:
        r= db.session.query(EmailResponse).filter_by(campaignId=campaignId).filter_by(response=x).count()
        responses[x]= r
